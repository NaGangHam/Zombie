...
public class PlayerHealth : LivingEntity {
    ...
    public override void OnDamage(float damage, Vector3 hitPoint, Vector3 hitDirection) {
        ...
        healthSlider.value = health; // 갱신된 체력으로 체력 슬라이더 반영 
    ...
    public override void Die() {
        ...
        playerAnimator.SetTrigger("Die"); // 애니메이터의 Die트리거를 발동시켜 사망 애니메이션 재생
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        // 사망하지 않는 경우에만 아이템 사용 가능
        if (!dead) {
            IItem item = other.GetComponent<IItem>(); // 충동한 상대방으로부터 IItem 컴포넌트 가져오기 시도
