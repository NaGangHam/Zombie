...
public class ZombieSpawner : MonoBehaviour {
    ...
    private void SpawnWave() {
        wave++; // 웨이브 1증가
        int spawnCount = Mathf.RoundToInt(wave* 1.5f); // 현재 웨이브 *1.5 를 반올림한 수만큼 좀비 생성
    ...
    private void CreateZombie() {
        ZombieData zombieData = zombieDatas[Random.Range(0, zombieDatas.Length)]; // 사용할 좀비 데이터 랜덤으로 결정
        Transform spawnPoint = spawnPoints[Random.Range(0, spawnPoints.Length)]; // 생성할 위치를 랜덤으로 결정
        Zombie zombie = Instantiate(zombiePrefab, spawnPoint.position, spawnPoint.rotation); // 좀비 프리팹으로부터 좀비생성
        zombie.Setup(zombieData); // 생성한 좀비의 능력치 설정
